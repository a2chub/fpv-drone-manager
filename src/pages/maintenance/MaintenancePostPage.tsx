...
import { useCreateMaintenancePost } from '@/hooks/useMaintenancePosts'
...
/**
 * 整備記録投稿作成ページ
 * `/maintenance/new` でアクセス
 */
export function MaintenancePostPage() {
  const navigate = useNavigate()
  const { user } = useAuth()
  const { data: drones, isLoading: isDronesLoading } = useDrones()
  const createPostMutation = useCreateMaintenancePost()
  ...
  const onSubmit = async (data: FormInputs) => {
    ...
    try {
      const postId = await createPostMutation.mutateAsync({
        ...
        droneImageUrl: drone.mainImageUrl || null,
        data: {
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <motion.button
              type="submit"
              disabled={isSubmitting || isUploading || createPostMutation.isPending}
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
